<script lang="ts">
import { computed, defineComponent, onDeactivated, onMounted, reactive, ref, watchEffect } from '@vue/runtime-core';
...
import { Calendar, Search, CirclePlus, Download, Upload } from '@element-plus/icons-vue';
...
export default defineComponent({
    ...
    setup(ctx) {
        ...
        const openTypes: ComN.openType[] = ['vscode', 'webStorm'];
        ...
        const list_ = computed(() => {
            ...
            return list.value.filter((_) => {
                return reg.test(_.key) || reg.test(_.title) || reg.test(_.paths.join(''));
            });
        });
        ...
        function exportClick() {
            let name = `vscodeOpenItemList@${new Date()
                ...
                .replace(/:/g, '_')
                .replace(/\s+/g, '_')}.json`;
            FileT.download(URL.createObjectURL(new File([new Blob([JSON.stringify(list.value)])], name)), name);
        }
        ...
        return {
            ...
            Download,
            defIcon,
            open_item_path: (...arg: Parameters<ComAC['open_item_path']>) => {
                ComAC.instance.open_item_path(...arg);
            },
            inputRef,
            openTypes,
        };
    ...
    <div class="home">
        <div class="content">
            <div class="nav">
                ...
                <div class="right">
                    <el-switch v-model="config.dark" @change="editConfig()" active-text="黑夜" inactive-text="白天" />
                </div>
            </div>
            <div class="content">
                <template v-if="show == 'list'">
                    <div class="top">
                        <div class="left">
                            <el-button style="margin-right: 20px" type="primary" @click="edit()" :icon="CirclePlus">
                                添加
                            </el-button>
                            <el-input
                                v-model="filterInput"
                        ...
                        <div class="right">
                            <el-button style="margin-right: 20px" @click="importClick()" :icon="Download"
                                >导入
                            </el-button>
                            <el-button style="margin: 0" @click="exportClick()" :icon="Upload">导出 </el-button>
                        </div>
                    </div>
                    <el-table border :data="list_" style="width: 100%">
                        <el-table-column show-overflow-tooltip width="120px" prop="key" label="key" />
                        <el-table-column show-overflow-tooltip width="120px" prop="openType" label="打开方式" />
                        ...
                        <el-table-column prop="title" label="标题">
                            <template #default="{ row }">
                                <span class="vscode_item" @click="open(row)" style="color: #0080cf">{{
                                    row.title
                                }}</span>
                            </template>
                        </el-table-column>
                            ...
                            <template #default="{ row }">
                                <div style="display: flex; flex-direction: column">
                                    <span
                                        ...
                                        :key="index"
                                        @click="open_item_path(item, row.openType)"
                                        >{{ item }}</span
                                    >
                                </div>
                            </template>
                        ...
                        <el-table-column fixed="right" label="操作" width="100">
                            <template #default="{ row }">
                                <el-button style="margin-left: 5px" link type="primary" size="small" @click="edit(row)"
                                    >编辑
                                </el-button>
                                <el-button
                ...
                <el-form label-suffix="：" v-if="show == 'config'" :model="config" label-width="180px">
                    <el-form-item label="端口号" prop="port">
                        <div style="display: flex; flex-direction: column">
                            <el-input-number v-model="config.port" :min="80" :max="65535" @change="editConfig()" />
        ...
        <el-dialog v-model="showDialog" title="添加/编辑项目" width="600px">
            <el-form ref="formEl" :model="formData" :rules="formDataRules" label-width="90px" label-suffix="：">
                ...
                <el-form-item label="打开方式" prop="openType">
                    <el-select v-model="formData.openType">
                        <el-option v-for="item in openTypes" :key="openTypes" :label="item" :value="item"> </el-option>
                    </el-select>
                </el-form-item>
                ...
                <el-form-item label="路径" prop="pathStr">
                    <el-input v-model="formData.pathStr" :rows="6" type="textarea" placeholder="请输入路径" />
                    <span>多个路径用换行符或;符号分隔</span>
                </el-form-item>
            </el-form>
            <template #footer>
                <span class="dialog-footer">
                    <el-button :loading="submitLoading" @click="showDialog = false">取消</el-button>
                    <el-button :loading="submitLoading" type="primary" @click="submit()"> 提交 </el-button>
                </span>
            </template>
        </el-dialog>
        <SecondConfirmation comType="danger" v-model:data="onRemove" :comF="remove" title="是否删除该项目" />
    </div>
</template>
